...
res = spark.sql("SELECT plate_id, registration_state, COUNT(*) \
           ...
           ORDER BY COUNT(*) DESC, plate_id asc LIMIT 20")
res.select(format_string('%s, %s\t%d',res.plate_id,res.registration_state,res['count(1)'])).write.save("task6-sql.out",format="text")
